...
namespace Post.Query.Infra.Converts
{
	public class EventJsonConverter : JsonConverter<BaseEvent>
	{
		...
		public override BaseEvent Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
		{
			...
			return typeDiscriminator switch
			{
				...
				nameof(CommentUpdatedEvent) => JsonSerializer.Deserialize<PostLikedEvent>(json, options),
				nameof(CommentRemovedEvent) => JsonSerializer.Deserialize<PostLikedEvent>(json, options),
				nameof(PostRemovedEvent) => JsonSerializer.Deserialize<PostRemovedEvent>(json, options),
				_ => throw new JsonException($"{typeDiscriminator} is not MulticastNotSupportedException yer!") 
			}; 
		}
